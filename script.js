...
function initAdmin(){
    ...
    const settingsBox=document.getElementById('admin-settings');
    ...
    unlockBtn.addEventListener('click',(event)=>{
        ...
        if(entered!==ADMIN_PASSWORD){
            error.textContent=entered===CALCULATOR_PASSWORD
                ? 'Це пароль входу в калькулятор. Для налаштувань потрібен інший пароль.'
    ...
    document.getElementById('admin-form').addEventListener('submit',e=>{
        e.preventDefault();
        if(settingsBox.classList.contains('hidden')) return;
        const map=adminMap();
        Object.entries(map).forEach(([id,key])=>settings[key]=Number(document.getElementById(id).value));
        saveSettings();
        closeAdmin();
