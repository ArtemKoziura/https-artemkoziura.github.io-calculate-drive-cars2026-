...
function initLogin(){ const overlay=document.getElementById('login-overlay'),app=document.getElementById('app'),form=document.getElementById('login-form'),input=document.getElementById('login-password'),error=document.getElementById('login-error'); form.addEventListener('submit',e=>{e.preventDefault(); if(input.value===ADMIN_PASSWORD){overlay.classList.add('hidden');app.classList.remove('is-locked');sessionStorage.setItem('drivecarsUnlocked','1');}else{error.textContent='Невірний пароль';input.select();}}); if(sessionStorage.getItem('drivecarsUnlocked')==='1'){overlay.classList.add('hidden');app.classList.remove('is-locked');} }
